...
@testset "Simplex Constructors" begin
    @testset "Basic Construction from Vertices" begin
        ...
        s2d = SimplicialContinuation.Simplex(vertices_2d)
        @test s2d.n == 2
        @test s2d.dims == 2
        ...
        s3d = SimplicialContinuation.Simplex(vertices_3d)
        @test s3d.n == 3
        @test s3d.dims == 3
        ...
        s_mixed = SimplicialContinuation.Simplex(mixed_vertices)
        @test eltype(s_mixed) == Float64

        # Test construction from matrix
        mat_vertices = [0 1 0; 0 0 1]
        s_mat = SimplicialContinuation.Simplex(mat_vertices)
        @test s_mat.n == 2
        @test s_mat.dims == 2
    ...
    @testset "Type-Specific Construction" begin
        ...
        s_int = SimplicialContinuation.Simplex{Int}(vertices)
        @test eltype(s_int) == Int
        @test s_int.vertices[:, 1] == [0, 0]

        s_float = SimplicialContinuation.Simplex{Float64}(vertices)
        @test eltype(s_float) == Float64
        @test s_float.vertices[:, 1] == [0.0, 0.0]

        # Test construction with different numeric types
        s_int32 = SimplicialContinuation.Simplex{Int32}(vertices)
        @test eltype(s_int32) == Int32
    end

    @testset "Uninitialized Construction" begin
        # Test uninitialized construction with square dimensions
        s_uninit = SimplicialContinuation.Simplex{Float64}(undef, 2, 2)
        @test s_uninit.n == 2
        @test s_uninit.dims == 2
        @test size(s_uninit.vertices, 2) == 3
        @test size(s_uninit.vertices, 1) == 2

        # Test uninitialized construction with different dimensions
        s_uninit_rect = SimplicialContinuation.Simplex{Int}(undef, 2, 3)
        @test s_uninit_rect.n == 2
        @test s_uninit_rect.dims == 3
        @test size(s_uninit_rect.vertices, 2) == 3
        @test size(s_uninit_rect.vertices, 1) == 3
    end

    @testset "Copy Constructor" begin
        # Test copying between same types
        original = SimplicialContinuation.Simplex([[1, 2], [3, 4], [5, 6]])
        copied = SimplicialContinuation.Simplex(original)
        ...
        s_int = SimplicialContinuation.Simplex([[1, 2], [3, 4], [5, 6]])
        s_float = SimplicialContinuation.Simplex{Float64}(s_int)
        @test eltype(s_float) == Float64
    ...
    @testset "Error Handling" begin
        ...
        @test_throws ArgumentError SimplicialContinuation.Simplex(bad_vertices)

        # Test empty vertices
        @test_throws BoundsError SimplicialContinuation.Simplex([])
    end
end
